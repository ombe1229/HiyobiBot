...
class Embeds:
    ...
    Help.add_field(name=';검색 K N', value='히요비에 K를 검색하여 K번째 페이지를 불러옵니다.', inline=False)
    ...
    Help.add_field(name=':warning: 경고', value='모든 명령어는 "연령 제한 채널"이 아니어도 정상 작동합니다.\n주의하세요.', inline=False)


    Wait = discord.Embed(title=':file_folder: 정보를 불러오는 중입니다...',description='잠시만 기다려주세요...', color=0xababab)
    ...
    NoResult.add_field(name='검색 결과가 없습니다.', value='번호가 정확한지 다시 한번 확인해주세요.', inline=False)

    Error = discord.Embed(title=':warning: 오류', color=0xff0000)
    Error.set_thumbnail(url=thumbnail)
    Error.add_field(name='오류가 발생했습니다.', value='지속적으로 오류 발생 시 ombe#7777으로 문의해주세요.', inline=False)
    ...
    Support = discord.Embed(title=':envelope: HiyobiBot 지원', url='https://discord.gg/3zSTSNT', description='위 링크를 눌러 HiyobiBot 디스코드 서버에 접속하거나, ombe#7777 DM으로 지원이 가능합니다.', color=0xff0000)
